...
import { CardNotPresentBlueMaxPayPayment } from "@pos_card_not_present/js/card_not_present_popup";
import { ErrorPopup } from "@point_of_sale/app/errors/popups/error_popup";
import { _t } from "@web/core/l10n/translation";

export class PaymentCardNotPresent extends PaymentInterface {
    setup() {
    ...
    async send_payment_request(cid) {
        ...
        if (PaymentLine.amount <= 0) {
            ...
            const { confirmed } = this.env.services.popup.add(CardNotPresentBlueMaxPayPayment, {
    ...
    pending_bluemaxpay_line() {
        return this.pos.get_order().paymentlines.find(
            paymentLine => paymentLine.payment_method.use_payment_terminal === 'card_not_present' && (!paymentLine.is_done()));
    }
