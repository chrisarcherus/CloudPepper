...
class AmazonProcessImportExport(models.TransientModel):
    ...
    def amz_import_fbm_shipped_orders(self):
        ...
        :return: shipped.order.data.queue.ept() tree or form view or True
        """
        sale_order_obj = self.env[SALE_ORDER]
        ...
        if not data_queue_list:
            return True
        return self.amz_return_client_action_ept('FBM Shipped Orders')

    @staticmethod
    def amz_return_client_action_ept(title, message=''):
        """
        Define this method for return client action for FBM shipped, Un Shipped orders and Map
        odoo/amazon products.
        :param: title: str
        :param: message: str
        :return: ir.actions.client()
        """
        client_message = "Import Operation Completed Successfully!"
        if message:
    ...
    def amz_import_fbm_missing_unshipped_orders(self):
        """
        This method Import FBM UnShipped Orders from Amazon to odoo.
        :return: shipped.order.data.queue.ept() tree or form view or True
        """
        sale_order_obj = self.env[SALE_ORDER]
        data_queue_list = sale_order_obj.import_fbm_shipped_or_missing_unshipped_orders(
        ...
        if not data_queue_list:
            return True
        return self.amz_return_client_action_ept('FBM UnShipped Orders')

    # commented below method because Import UnShipped Orders report is deprecated
    # we Uses unshipped orders data queue process for process unshipped orders
    ...
    def amz_removal_order_report(self):
        ...
        removal_order_request_report_record = removal_order_request_report_record.create(
            removal_order_request_report_vals)
    ...
    def amz_import_products(self):
        ...
        return self.amz_return_client_action_ept('Map Odoo/Amazon Product', message)
