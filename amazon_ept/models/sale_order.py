...
class SaleOrder(models.Model):
    ...
    def import_fba_pending_sales_order(self, seller, marketplaceids, updated_after_date):
        """
        Define this method for Import FBA Pending Sales Order From Amazon
    ...
    def process_fba_pending_sale_order_response(self, seller, kwargs, result):
        ...
        :param: seller: amazon.seller.ept()
        :param: kwargs: dict {}
        :param: result: response from amazon
        :return: True
        """
        ...
        if next_token:
            # We have created list of Dictwrapper now we create orders into system
    ...
    def check_amazon_fba_draft_orders(self, seller, marketplaceids, instance_ids):
        ...
        :param: seller: amazon.seller.ept()
        :param: marketplaceids: list of amazon market places
        :param: instance_ids: amazon.instance.ept()
        :return: str, sale.order(), sale.order()
        """
        message = ''
    ...
    def cancel_amazon_fbm_pending_sale_orders(self, seller, marketplaceids, instance_ids):
        ...
        :param: seller: amazon.seller.ept()
        :param: marketplaceids: list of marketplaces
        :param: instance_ids: list of amazon.instance.ept()
        :return: True
        """
    ...
    def check_amazon_fba_and_fbm_cancel_order(self, seller, marketplaceids, updated_after,
        ...
        :return: response from amazon api
        """
        log_line_obj = self.env[COMMON_LOG_LINES_EPT]
        ...
        for order_line_wrapper_obj in list_of_order_lines_wrapper:
            ...
            for order_line in order_lines:
                skip_order, product_details = self.with_context(
                    ...
                    order_line, product_details, instance, amazon_order_ref)
                if skip_order:
                    break
    ...
    def create_or_find_amazon_product(self, order_details, product_details, instance, amazon_order_ref,
                                      queue_order=False):
        """
    ...
    def amz_search_or_create_odoo_product(self, instance, amazon_order_ref, fulfillment_by,
                                          order_details, skip_order, queue_order=False):
        """
        ...
        :param : queue_order : shipped.order.data.queue.ept() or False
        ...
        if odoo_product:
            ...
            if queue_order:
                res_model_name = 'shipped.order.data.queue.ept'
            message = 'Product %s not found for %s instance' % (seller_sku, instance.name)
            common_log_line_obj.create_common_log_line_ept(message=message, model_name=res_model_name,
                                                           order_ref=amazon_order_ref, default_code=seller_sku,
                                                           fulfillment_by=fulfillment_by, module='amazon_ept',
                                                           operation_type='import', mismatch_details=True,
                                                           res_id=queue_order.id if queue_order else False,
                                                           amz_seller_ept=instance.seller_id and
                                                                          instance.seller_id.id or False,
                                                           amz_instance_ept=instance and instance.id or False)
        else:
    ...
    def amz_create_order_lines(self, order, instance, order_details, dict_product_details):
        """
        Define this method for create sale order line for FBM Shipped and Un Shipped orders.
        :param: order: sale.order()
        :param: instance: amazon.instance.ept()
        :param: order_details: dict {}
        :param: dict_product_details: dict {}
        :return: True
        """
        taxargs = {}
    ...
    def get_fbm_next_token_orders(self, seller, next_token):
        ...
        :param: seller: amazon.seller.ept()
        :param: next_token: next token for orders request in amazon
    ...
    def process_shipped_or_missing_unshipped_lines_ept(self, instance, order, line, list_of_shipped_order_lines,
        ...
        :param: instance: amazon.instance.ept()
        :param: order: order response
        :param: line: shipped.order.data.queue.line.ept()
        :param: list_of_shipped_order_lines: list of order lines
        :param: queue_order: shipped.order.data.queue.ept()
        :return: True
        """
    ...
    def process_shipped_or_missing_unshipped_order_ept(self, instance, order, order_lines, line_state, queue_order):
        """
        Define this method for create sale order and order lines as per order response.
        :param: instance: amazon.instance.ept()
        :param: order: order response
        :param: order_lines: list of order lines
        :param: line_state: shipped.order.data.queue.line.ept() state
        :param: queue_order: shipped.order.data.queue.ept()
        :return: sale.order(), shipped.order.data.queue.line.ept() state
        """
        partner = {}
        state_dict = dict()
        country_dict = dict()
        ...
        cancel_requests = [x.get('BuyerRequestedCancel') for x in order_lines if eval(x.get(
            'BuyerRequestedCancel', {}).get('IsBuyerRequestedCancel', 'false').capitalize())]
        skip_order = False
        for order_line in order_lines:
            order_skip, product_details = self.create_or_find_amazon_product(
                order_line, dict_product_details, instance, amazon_order_ref, queue_order)
            if order_skip:
        ...
        for order_line in order_lines:
            ...
            if not sales_order or sales_order.amz_order_reference != amazon_order_ref:
                if order.get('BuyerInfo', {}).get('BuyerEmail', ''):
                    ...
                    partner = self.get_amz_fbm_partners(customer_vals, state_dict, country_dict, instance)
                if partner:
                    ordervals = self.create_amazon_shipped_or_unshipped_order_vals(instance, partner, order)
                    if cancel_requests:
                        ordervals.update({
                            'buyer_requested_cancellation': eval(cancel_requests[0].get(
                                'IsBuyerRequestedCancel', 'false').capitalize()),
    ...
    def get_fbm_orders(self, seller, marketplaceids, orderstatus, updated_after_date,
        ...
        :param: seller: amazon.seller.ept()
        :param: marketplaceids: list of marketplaces
        :param: orderstatus: list of order status
        :param: updated_after_date: datetime
        :param: updated_before_date: datetime
    ...
    def create_shipped_or_missing_unshipped_queue(self, datas, instance_dict, seller, data_queue):
        """
        This method will create shipped and Un-Shipped orders data queue lines.
        :param: datas: list of amazon orders response
    ...
    def auto_process_shipped_order_queue_line(self, data_queue):
        """
        This method will activate the cron to auto process shipped order queue lines.
        :param: data_queue: shipped.order.data.queue.ept()
        :return: True
        """
    ...
    @staticmethod
    def get_marketplaceids_and_request_date(seller, instance, updated_after_date):
        """
        This method will return the marketplaces and request after date to get orders
        :param: seller: amazon.seller.ept()
        :param: instance: amazon.instance.ept()
        :param: updated_after_date: datetime
        return: list of marketplaces, datetime object
        """
    ...
    def import_fbm_shipped_or_missing_unshipped_orders(self, seller, instance, updated_after_date, orderstatus,
        ...
        result = self.get_fbm_orders(seller, marketplaceids, orderstatus, updated_after_date,
                                     updated_before_date)
        next_token = result.get('NextToken', '')
        data_queue = self.amz_create_shipped_unshipped_data_queue(seller)
    ...
    def amz_create_shipped_unshipped_data_queue(self, seller):
        """
        Define this method for create data queue for shipped or unshipped orders.
        :param: seller: amazon.seller.ept()
        :return: shipped.order.data.queue.ept()
        """
        shipped_order_data_queue_obj = self.env['shipped.order.data.queue.ept']
        return shipped_order_data_queue_obj.create({'amz_seller_id': seller.id})
    ...
    def prepare_tracking_no_dict_with_qty(move):
        ...
        for move_line in move.move_line_ids.filtered(lambda line: line.picked):
    ...
    def get_amz_fbm_partners(self, vals, state_dict, country_dict, instance):
        """
    ...
    def search_amazon_fbm_invoice_and_partner_ept(self, instance, buyer_name, new_partner_vals):
        ...
        :param: instance: amazon.instance.ept()
        :param: buyer_name: buyer name - str
