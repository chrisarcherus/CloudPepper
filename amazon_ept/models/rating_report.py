...
class RatingReportHistory(models.Model):
    ...
    report_id = fields.Char('Report ID', readonly=True)
    ...
    amz_rating_report_ids = fields.One2many(RATING_RATING, 'amz_rating_report_id', string="Ratings (Amz)")
    ...
    def list_of_rating(self):
        """
        This Method relocates list of amazon rating.
        :return:
        """
        rating_obj = self.env[RATING_RATING]
    ...
    def create_amazon_report_attachment(self, result):
        ...
        self.message_post(body=_("Rating Report Downloaded"), attachment_ids=attachment.ids)
    ...
    def check_rating_report_configuration_ept(self):
        ...
        if not self.seller_id:
            message = "Seller is not defined for processing report"
            if not self._context.get('is_auto_process', False):
                raise UserError(_(message))
            common_log_line_obj.create_common_log_line_ept(
                message=message, model_name='rating.report.history', module='amazon_ept', operation_type='import',
    ...
    def process_rating_report(self):
        ...
        """
        self.ensure_one()
        # check is mismatch details found from the report configuration then skip the process
        if self.check_rating_report_configuration_ept():
            return True
        common_log_line_obj = self.env[COMMON_LOG_LINES_EPT]
        ...
        for row in reader:
            ...
            amazon_sale_order = sale_order_obj.search([
                ...
                ('amz_instance_id', 'in', self.seller_id.instance_ids.ids)], limit=1)
