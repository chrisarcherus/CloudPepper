...
class DeliveryCarrier(models.Model):
    ...
    def shipstation_rate_shipment(self, order):
        ...
        try:
            ...
            if response_data.status_code == 200:
                ...
                if shipstation_charge_id:
                    order.write({'shipstation_shipping_charge_ids': [(5, 0, 0)]})
    ...
    def generate_label_from_shipstation(self, picking, package_id=False, weight=False):
        ...
        total_weight = self.get_total_weight(package_id.shipstation_weight)
        request_data = {
        ...
        if len(shipstation_carrier_code) > 1 and self.shipstation_carrier_id.shipping_provider_id:
            if request_data.get('advancedOptions'):
                ...
                request_data.update({
                    "advancedOptions": {"billToParty": "my_other_account",
                                        "billToMyOtherAccount": self.shipstation_carrier_id.shipping_provider_id}
                })
        return request_data
        ...
        if date_order:
            ...
            request_data = {
                ...
                "billTo": {
                    ...
                    "postalCode": "%s" % (picking_receiver_id.zip[:5] or "") if picking_receiver_id.country_id.code == "US" else "%s" % (picking_receiver_id.zip or ""),
            ...
            if len(shipstation_carrier_code) > 1 and self.shipstation_carrier_id.shipping_provider_id:
                request_data.get('advancedOptions').update({"billToParty": "my_other_account",
                                                            "billToMyOtherAccount": self.shipstation_carrier_id.shipping_provider_id})
            if picking.shipment_weight:
                request_data.get('weight').update({"value": picking.shipment_weight})
            if picking.shipment_length:
                request_data.get('dimensions').update({"length": picking.shipment_length})
            if picking.shipment_width:
                request_data.get('dimensions').update({"width": picking.shipment_width})
            if picking.shipment_height:
                request_data.get('dimensions').update({"height": picking.shipment_height})
    ...
    def shipstation_get_tracking_link(self, pickings):
        ...
        for picking in pickings:
            ...
            if len(pickings.carrier_tracking_ref.split(',')) > 1:
                if self.shipstation_carrier_id and self.shipstation_carrier_id.code in ["ups", "UPS", "ups_walleted",
                                                                                        "UPS_WALLETED"]:
    ...
    def shipstation_mark_as_shipped_order(self, picking):
        if picking.sale_id.shipstation_order_id and picking.sale_id.carrierCode:
            req_data = {
                "orderId": int(picking.sale_id.shipstation_order_id),
                "carrierCode": '{}'.format(picking.sale_id.carrierCode)
            }
            try:
                response_data = self.api_calling_function("/orders/markasshipped", req_data)
                if response_data.status_code == 200:
                    responses = response_data.json()
                    _logger.info("Response Data: %s" % (responses))
                    order_id = responses.get('orderId')
                    ...
                    if order_id:
                        picking.shipstation_order_id = order_id
                        picking.shipstation_sale_order_number = order_number
                        picking.sale_id.orderStatus = 'shipped'
                        return True
                else:
                    error_code = "%s" % (response_data.status_code)
                    error_message = response_data.reason
